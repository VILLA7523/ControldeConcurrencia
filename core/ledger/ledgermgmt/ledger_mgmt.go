...
// ErrLedgerAlreadyOpened se lanza en una llamada a CreateLedger si ya se abrió un ledger con el ID dado
var ErrLedgerAlreadyOpened = errors.New("ledger already opened")

// ErrLedgerMgmtNotInitialized se lanza cuando se utiliza ledger mgmt antes de inicializarlo
var ErrLedgerMgmtNotInitialized = errors.New("ledger mgmt should be initialized before using")

// LedgerMgr gestiona los ledgers para todos los canales
...
// MetadataProvider proporciona metadatos para los paquetes de chaincode
type MetadataProvider interface {
	PackageMetadata(ccid string) ([]byte, error)
}

// Initializer encapsula todas las dependencias externas para el módulo de ledger
...
// NewLedgerMgr crea un nuevo LedgerMgr
...
// CreateLedger crea un nuevo ledger con el bloque génesis
...
func (m *LedgerMgr) OpenLedger(id string) (ledger.PeerLedger, error) {
	// Pseudo: b := fetch the number of the last block;
	b := m.fetchLastBlockNumber()

	// Pseudo: readset, writeset := simulate the contract invocation on Block b snapshot; 4.2 Snapshot Read
	readSet, writeSet := m.simulateContractInvocation(nil, b)

	// Imprime información de apertura del ledger
	logger.Infof("Abriendo ledger con ID = %s", id)

	m.lock.Lock()
	defer m.lock.Unlock()

	// Verifica si el ledger ya está abierto
	...
	// Almacena el ledger abierto
	m.openedLedgers[id] = l

	// Imprime información de apertura del ledger
	logger.Infof("Ledger abierto con ID = %s", id)

	// Devuelve el ledger abierto
	return &closableLedger{
		ledgerMgr:  m,
		id:         id,
		PeerLedger: l,
